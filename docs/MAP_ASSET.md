...
- Bei ausdrücklich als verzögert berichteten Jahreskennzahlen, insbesondere installierter Leistung und der Inventaremissions-BIP-Relation, darf die Karte auf das jüngste verfügbare frühere Jahr ab 2015 zurückfallen. Angefordertes Jahr und tatsächlich verwendetes Datenjahr werden getrennt ausgewiesen; die Datenbank wird dabei nicht fortgeschrieben oder aufgefüllt.
- Werte können auf der Karte ein- und ausgeblendet werden. Die Legende zeigt das Land mit Minimum und Maximum sowie den Atlas-Durchschnitt. SVG- und PNG-Export bilden den aktuellen Zustand einschließlich Titel, Zeitraum, Einheit, Farbskala und Legende ab.
- Ein Kartenklick setzt einen dauerhaften Länderfokus, ohne die Länderauswahl des Zeitvergleichs zu verändern. Ein Klick auf den umgebenden Kartenhintergrund löst diesen Fokus wieder.
...
- Die analytische Karte verwendet ausschließlich lokale Assets. Der optionale Modus `Europa Overload` lädt separat zugeschaltete, attribuierte Postkartenbilder von Wikimedia Commons und verändert weder Geometrie noch Kartendaten. Vollbildgalerie, Netzwerkzugriffe und öffentliche Daumenstimmen sind in [EUROPA_OVERLOAD.md](EUROPA_OVERLOAD.md) beschrieben.
