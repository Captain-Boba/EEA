...
Ember remains the sole source for electricity-system metrics and national wholesale prices. Eurostat supplies annual socioeconomic denominators as well as installed capacity, retail-price components, gross electricity trade, and battery-electric passenger-car values. Annual per-capita generation metrics combine an Ember numerator only with a positive Eurostat population value from the same year; they are unavailable in monthly views. Household price totals and components are presented in ct/kWh after a deterministic conversion from the imported EUR/MWh observations, while non-household values remain in EUR/MWh. Battery-Charts and JRC storage data are exposed as separately dated stock values; the JRC hydro inventory and EEA emissions inventory remain distinct datasets. None of these auxiliary sources silently fills Ember gaps.
